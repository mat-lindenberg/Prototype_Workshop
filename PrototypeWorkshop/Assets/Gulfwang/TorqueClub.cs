...
public class TorqueClub : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            golfClub.transform.rotation = Quaternion.Euler(0, 0, restingAngle);
            power = 0;
            mySwingState = swingMode.Nothing;
        }
    ...
    public void RecieveBall(GameObject g)
    {
        if (mySwingState == swingMode.Releasing)
        {
            Debug.Log("should be hitting that ball");
            g.GetComponent<Rigidbody2D>().AddForce(new Vector3(1,0,0) * power, ForceMode2D.Impulse);
        }
    }
